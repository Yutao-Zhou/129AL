...
#The width of the line are 20 that why y has a range from 38 to 58
a=35
for j in range(a,55,1):
    for i in range(52,452,1):
        pvals[i,j,0] = 0x00  # red
        pvals[i,j,1] = 0x00    # green
        pvals[i,j,2] = 0xff  # blue
# The following loop creat the vertical side of the triangle that has a length of 310 pixel so that it would over lap with the slope side
#As you can see the x end at 462 so they over lap, and y start at 38 so they over lap and are connected.
#The width of the line are 20 that why x has a range from 442 to 452
for j in range(a,330,1):
    for i in range(442,462,1):
        pvals[i,j,0] = 0x00  # red
        pvals[i,j,1] = 0x00    # green
        pvals[i,j,2] = 0xff  # blue
...
#The width of the line are 20 that why I make the lineear equation have the form y=ax+b where b is o here and is mving the line in vertical direction. So that the line has a weigth of 10
for o in range(20):    
    for i in range(50,462,1):
        j=int((3*i/4)-o)    
        pvals[i,j,0] = 0x00  # red
        pvals[i,j,1] = 0x00  # green
        pvals[i,j,2] = 0xff  # blue

#making the left corner perfect
for o in range(20):    
    for i in range(50,462,1):
        j=int((3*i/4)+o)    
        pvals[i,j,0] = 0xff  # red
